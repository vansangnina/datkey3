...
function upload_image($file, $extension, $folder, $newname=''){
	if(isset($_FILES[$file]) && !$_FILES[$file]['error']){
		...
		$filename_1 = htmlspecialchars($_FILES[$file]["tmp_name"]);
		
		if($newname=='' or file_exists($folder.$filename_))
			for($i=0; $i<100; $i++){
		...
		if (!copy($filename_1, $folder.$filename_))	{
			if ( !move_uploaded_file($filename_1, $folder.$filename_))				        { 
				return false;
			}
		}
